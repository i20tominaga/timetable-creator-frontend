...
const EditTimetable = () => {
        ...
        (day: string, classLabel: string, period: number | undefined) => {
            ...
            if (selectedClass === null) {
                ...
                setTimetable((prevTimetable) => {
                    ...
                    // 空きコマも対象に含めるため、インデックスが-1でもスワップを続行
                    const classEntry1: ClassEntry = classIndex1 !== -1
                        ? newTimetable.Days[dayIndex1].Classes[classIndex1]
                        : {
                            Subject: "空きコマ",
                            Rooms: [],
                            Instructors: [],
                    ...
                    const classEntry2: ClassEntry = classIndex2 !== -1
                        ? newTimetable.Days[dayIndex2].Classes[classIndex2]
                        : {
                            Subject: "空きコマ",
                            Rooms: [],
                            Instructors: [],
                    ...
                    if (classIndex1 !== -1) {
                        ...
                        newTimetable.Days[dayIndex1].Classes.push(classEntry2);
                    }

                    if (classIndex2 !== -1) {
                        newTimetable.Days[dayIndex2].Classes[classIndex2] = classEntry1;
                    } else {
                        newTimetable.Days[dayIndex2].Classes.push(classEntry1);
                    }
    ...
    const generateTimetableByClasses = (days: TimetableDay[] | undefined) => {
        ...
        // 空きコマをダミーのClassEntryとして埋める
        for (const classLabel in timetableByClasses) {
            for (const day in timetableByClasses[classLabel]) {
                timetableByClasses[classLabel][day] = timetableByClasses[classLabel][day].map((entry, index) => {
                    return entry ?? {
                        Subject: "空きコマ",
                        Rooms: [],
                        Instructors: [],
